...
update_status ModulePlayer::Update()
{
	...
	App->render->Blit(App->TEX->sprites, position.x, position.y, &r);
	...
	if (move2 == false) {
		detectionlive = 1;
		App->render->Blit(App->TEX->JANE, position.x + 10, position.y, &help);
		App->render->Blit(App->TEX->JANE, position.x - 2, position.y + 5, &dead2player);
		current_animation = &invisible;
		contdead = 1;
	}
	...
	if (App->player2->torevive != 0) {
		if (App->input->keyboard[SDL_SCANCODE_9] == KEY_STATE::KEY_REPEAT || App->input->buttonX == KEY_STATE::KEY_REPEAT) {
			...
			if (App->player2->revTime <= 30 && App->player2->revTime >= 1) {
				//if (revTime == 2)
					App->audio->play_fx11();
				App->render->Blit(App->TEX->JANE, App->player2->position.x - 5, App->player2->position.y, &App->player2->hpRev1);
			}
			else if (App->player2->revTime <= 60 && App->player2->revTime > 30) {
				//if(revTime ==32)
					App->audio->play_fx12();
				App->render->Blit(App->TEX->JANE, App->player2->position.x - 5, App->player2->position.y, &App->player2->hpRev2);
			}
			else if (App->player2->revTime <= 100 && App->player2->revTime > 60) {
				//if(revTime == 62)
					App->audio->play_fx13();

				App->render->Blit(App->TEX->JANE, App->player2->position.x - 5, App->player2->position.y, &App->player2->hpRev3);
			}
			else if (App->player2->revTime <= 130 && App->player2->revTime > 100) {
			//	if(revTime == 102)
					App->audio->play_fx14();

				App->render->Blit(App->TEX->JANE, App->player2->position.x - 5, App->player2->position.y, &App->player2->hpRev4);
			}
