...
update_status ModuleEnemies::PostUpdate()
{
	...
	for (uint i = 0; i < MAX_ENEMIES; ++i)
	{
		...
		{
			if ((enemies[i]->position.y > ((App->scene_1->current_start_pos + 114) - App->scene_1->cont + 88) + SPAWN_MARGIN))
	...
	for (uint i = 0; i < MAX_ENEMIES; ++i)
	{
		if (c1->type == COLLIDER_PRISONER) { //PRISONERS
			...
			if (enemies[i] != nullptr && enemies[i]->GetCollider() == c1)
			{
				...
				else if (c1->type == COLLIDER_ENEMY  && c2->type != COLLIDER_PLAYER && c2->type != COLLIDER_PLAYER2) {
